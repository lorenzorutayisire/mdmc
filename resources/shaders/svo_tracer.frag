...
// Took from: https://github.com/AdamYuan/SparseVoxelOctree/blob/master/shaders/octree_tracer.frag
...
layout(binding = 3, r32ui) readonly uniform uimageBuffer uuOctree;
...
bool RayMarchLeaf(vec3 o, vec3 d, out float o_t, out vec3 o_color, out vec3 o_normal) {
	...
	while( scale < STACK_SIZE ) {
		++iter;
		if(cur == 0u) cur = imageLoad(uuOctree, int(parent + ( idx ^ oct_mask ))).r;
...
void main() {
	...
	oFragColor = vec4( hit ? color : vec3(0), 1 );
}
